...
class FilterTab extends Component {
    ...
    */
    ...
    findResults = () => {
        ...
        const queryRef = firestore().collection('userInfo')
        ...
        firestore().collection('queries').doc('query' + useruid).delete()
        ...
        if (this.state.gender != 'Any') {
            queryRef.where( 'gender', '==', this.state.gender).get().then(snapshot => {
                ...
                snapshot.forEach(doc => {
                    ...
                    firestore().collection('queries').doc('query' + useruid).set({
                        queriedBy: auth().currentUser.uid
                    })
                    firestore().collection('queries').doc('query' + useruid).collection('users').doc(doc.id).set({
                        userArray: doc.data(),
